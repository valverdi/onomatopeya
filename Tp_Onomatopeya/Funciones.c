...
void pedirCantidadJugadores(SDL_Renderer* renderer, TTF_Font* font, int* cantidadJugadores) {
    ...
    // Capturar entrada
    int salirCaptura = 0;
    char entrada[3] = {0}; // Para 2 d�gitos como m�ximo
    int indice = 0;
    ...
    while (!salirCaptura) {
        ...
        while (SDL_PollEvent(&e)) {
            ...
            if (e.type == SDL_TEXTINPUT) {
                if (indice < 2) { // No m�s de 2 caracteres
    ...
    *cantidadJugadores = atoi(entrada); // Convertir entrada a n�mero
}
...
void pedirNombres(SDL_Renderer* renderer, TTF_Font* font, int cantidadJugadores, t_lista* p) {
    ...
    for (i = 0; i < cantidadJugadores; i++) {
        ...
        // Limpiar el �rea derecha antes de dibujar
        SDL_SetRenderDrawColor(renderer, 33, 33, 33, 255);
        SDL_RenderFillRect(renderer, &areaDerecha);
        ...
        // Obtener el tama�o del texto de la pregunta para centrarlo
        ...
        while (!salirCaptura) {
            ...
            // Limpiar el �rea derecha antes de redibujar
            SDL_SetRenderDrawColor(renderer, 33, 33, 33, 255);
            SDL_RenderFillRect(renderer, &areaDerecha);
            ...
            // Presentar la actualizaci�n de la pantalla
            SDL_RenderPresent(renderer);
        }
